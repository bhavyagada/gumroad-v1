<script>
  import { goto } from "$app/navigation";

  // TODO: extract everything later into specific states
  let show_error = false;
  let error_message = "";
  let number_of_days = 0;
  let s = 's';
  let chart_max = 0;
  let show_chart = false;
  let chart_numbers = 0;
  let last_seven_days_purchase_total = 0;
  let last_month_purchase_total = 0;
  let purchase_total = 0;
  let chart_src = `http://chart.apis.google.com/chart?chxr=0,0,${chart_max}&chf=bg,s,ffffff&chxt=y&chbh=a&chs=640x225&chco=CC333F,EB6841&cht=bvg&chds=0,${chart_max}&chd=t:${chart_numbers}`;
</script>

<div class="clear-both w-3/5 min-w-[800px] max-w-[960px] overflow-hidden mx-auto my-0 px-0 py-8">
  <div class="flex justify-between">
    <div class="bg-[#f9f9f9] w-[258px] h-[129px] border border-2 border-dashed border-[#eee] rounded-xl mr-5">
      <button on:click={() => goto("/add")} class="bg-[#f2f2f2] w-full font-bold text-lg text-[#0e7bba] p-4 rounded-tl-[15px] rounded-tr-[15px] border-t-2 border-t-[#eee] border-0 border-dashed hover:underline hover:text-[#0a547f] hover:cursor-pointer">+ Add new link</button>
      <p class="clear-both overflow-hidden text-center my-4">You don't have any links.</p>
    </div>
    <div class="bg-[#f9f9f9] w-[680px] pl-5 pt-5 pb-2.5 px-0 rounded-[15px]">
      {#if show_error}
        <h3 class="error text-black text-xl mx-0 mt-1 mb-0">{ error_message }</h3>
      {:else}
        <h3 class="text-black font-medium text-xl mx-0 mt-1 mb-0">Last {number_of_days} day{s}:</h3>
      {/if}

      <div class="pt-5 pb-0">
        {#if show_chart}
          <img src={chart_src} width="640" height="225" alt="Sales Chart" />
        {:else}
          <p>Wait a few days and a chart will show up here!</p>
        {/if}
      </div>
      <div class="mx-0 mt-2.5 mb-0">
        <h4 class="font-medium text-black text-xl mt-6 mb-3 mx-0">History:</h4>
        <p class="mx-0 my-1"><strong class="text-[#444]">${ last_seven_days_purchase_total || "0.00" }</strong> in the past 7 days.</p>
        <p class="mx-0 my-1"><strong class="text-[#444]">${ last_month_purchase_total || "0.00" }</strong> in the past month.</p>
        <p class="mx-0 my-1"><strong class="text-[#444]">${ purchase_total || "0.00" }</strong> in total.</p>
      </div>
      <div class="rainbow bar" id="loading-bar"></div>
    </div>
  </div>
</div>